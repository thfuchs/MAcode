...
```{r A_data, include=FALSE}
# Read Data
acc_ebit_h2_baselines <- readRDS("../acc_ebit_h2_baselines.rds")
...
### (i) $H^2_A$ Accuracy Naive + SNaive + Drift + Holt + LSTM + GRU
...
\newpage
### (ii) $H^2_A$ Rank Naive + SNaive + Drift + Holt + LSTM + GRU

#### Mean of Ranks per Split
```{r mean_rank, fig.height = 10}
acc_ebit_h2_rankavg <- readRDS("../acc_ebit_h2_baselines_rankavg.rds")
...
\newpage
#### Stacked Ranks Baselines vs. LSTM & GRU (only)
```{r rank_stack_01}
...
#### Stacked Ranks Baselines vs. LSTM & GRU (incl. ARIMA, Simple, ARNN)
```{r rank_stack_02}
# Read Data
...
# Plot
ggplot(
  data = acc_ebit_01_rank_melt[type %in% c("Naive", "Snaive", "Drift", "Holt", "LSTM", "GRU")],
