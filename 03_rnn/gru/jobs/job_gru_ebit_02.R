  ...
  purrr::possibly(function(x) {
    ...
    if (!is.null(result)) {
      RDStoS3(
        ...
        s3_prefix = "gru/predict/ebit/"
      )
      tmp_files <- list.files(
        tmp,
        pattern = sprintf("\\w+_ebit_gru_%s_\\w+\\.hdf5$", x),
        full.names = TRUE
      )
      hdf5toS3(files = tmp_files, s3_prefix = "gru/models/ebit/")
    }
    if (current <= overall) toSlack(paste0(
      "GRU EBIT: Predicted ", x, "\n",
...
if (length(fc_ebit_rnn_predict) > 0) {
  saveRDS(fc_ebit_rnn_predict, file = "03_rnn/gru/results/fc_ebit_rnn_predict.rds", compress = "xz")
  RDStoS3(data = fc_ebit_rnn_predict, filename = "fc_ebit_rnn_predict.rds", s3_prefix = "gru/")
  toSlack("GRU EBIT prediction finished")
} else toSlack("Error: GRU EBIT prediction failed")
