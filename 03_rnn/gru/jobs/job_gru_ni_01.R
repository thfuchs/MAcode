...
tuning_results <- furrr::future_map(
  ...
  purrr::possibly(function(x) {
    ...
    if (!is.null(result)) RDStoS3(
      ...
      s3_prefix = "gru/bayes/ni/"
    )
    if (current <= overall) toSlack(paste0(
      "GRU Net Income Bayes Optimization: Finished ", x, "\n",
...
if (length(fc_ni_rnn_bayes) > 0) {
  saveRDS(fc_ni_rnn_bayes, file = "03_rnn/gru/results/fc_ni_rnn_bayes.rds", compress = "xz")
  RDStoS3(data = fc_ni_rnn_bayes, filename = "fc_ni_rnn_bayes.rds", s3_prefix = "gru/")
  toSlack("GRU Net Income Bayes Optimization finished")
} else toSlack("GRU Net Income Bayes Optimization failed")
