...
fc_ni_rnn_bayes <- readRDS("03_rnn/gru/results/fc_ni_rnn_bayes.rds")
  ...
  purrr::possibly(function(x) {
    ...
    if (!is.null(result)) {
      RDStoS3(
        ...
        s3_prefix = "gru/predict/ni/"
      )
      tmp_files <- list.files(
        tmp,
        pattern = sprintf("\\w+_ni_gru_%s_\\w+\\.hdf5$", x),
        full.names = TRUE
      )
      hdf5toS3(files = tmp_files, s3_prefix = "gru/models/ni/")
      file.remove(tmp_files)
    }
    if (current <= overall) toSlack(paste0(
      "GRU Net Income: Predicted ", x, "\n",
...
if (length(fc_ni_rnn_predict) > 0) {
  saveRDS(fc_ni_rnn_predict, file = "03_rnn/gru/results/fc_ni_rnn_predict.rds", compress = "xz")
  RDStoS3(data = fc_ni_rnn_predict, filename = "fc_ni_rnn_predict.rds", s3_prefix = "gru/")
  toSlack("GRU Net Income prediction finished")
} else toSlack("Error: GRU Net Income prediction failed")
