...
fc_eps_rnn_predict <- readRDS("03_rnn/lstm/results/fc_eps_rnn_predict.rds")

stopifnot(all(names(fc_eps_rnn_bayes) == names(fc_eps_rnn_predict)))

companies <- names(fc_eps_rnn_predict)
...
if (length(purrr::compact(fc_eps_rnn_eval)) > 0) {
  saveRDS(fc_eps_rnn_eval, file = "03_rnn/lstm/results/fc_eps_rnn_eval.rds", compress = "xz")
  toSlack("LSTM EPS evaluation finished")
} else toSlack("Error: LSTM EPS evaluation failed")
