...
tuning_results <- furrr::future_map(
  ...
  purrr::possibly(function(x) {
    ...
    if (!is.null(result)) RDStoS3(
      ...
      s3_prefix = "lstm/bayes/eps/"
    )
    if (current <= overall) toSlack(paste0(
      "LSTM EPS Bayes Optimization: Finished ", x, "\n",
...
if (length(fc_eps_rnn_bayes) > 0) {
  saveRDS(fc_eps_rnn_bayes, file = "03_rnn/lstm/results/fc_eps_rnn_bayes.rds", compress = "xz")
  RDStoS3(data = fc_eps_rnn_bayes, filename = "fc_eps_rnn_bayes.rds", s3_prefix = "lstm/")
  toSlack("LSTM EPS Bayes Optimization finished")
} else toSlack("LSTM EPS Bayes Optimization failed")
