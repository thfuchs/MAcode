...
fc_eps_rnn_bayes <- readRDS("03_rnn/lstm/results/fc_eps_rnn_bayes.rds")
  ...
  purrr::possibly(function(x) {
    ...
    if (!is.null(result)) {
      RDStoS3(
        ...
        s3_prefix = "lstm/predict/eps/"
      )
      tmp_files <- list.files(
        tmp,
        pattern = sprintf("\\w+_eps_lstm_%s_\\w+\\.hdf5$", x),
        full.names = TRUE
      )
      hdf5toS3(files = tmp_files, s3_prefix = "lstm/models/eps/")
      file.remove(tmp_files)
    }
    if (current <= overall) toSlack(paste0(
      "LSTM EPS: Predicted ", x, "\n",
...
if (length(fc_eps_rnn_predict) > 0) {
  saveRDS(fc_eps_rnn_predict, file = "03_rnn/lstm/results/fc_eps_rnn_predict.rds", compress = "xz")
  RDStoS3(data = fc_eps_rnn_predict, filename = "fc_eps_rnn_predict.rds", s3_prefix = "lstm/")
  toSlack("LSTM EPS prediction finished")
} else toSlack("Error: LSTM EPS prediction failed")
